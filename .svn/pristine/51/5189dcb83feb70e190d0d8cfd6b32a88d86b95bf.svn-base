...
# Nastavi seme za spremenljivko RANDOM
RANDOM=42
...
function drevo()
{
	# Shrani naslov, maksimalno globino in trenutno globino.
	local address="$1"
	local max_depth=$2
	local current_depth=$3

	# Izracunaj stevilo znakov -, ki jih je pri tej globini potrebno izpisati (napacno poimenovana spremenljivka).
	local brackets=""
	for (( i = 0; i < current_depth; i++ )) ; do
		local brackets="$brackets----"
	done

	# Pridobi datoteke na podanem naslovu in jih shrani v tabelo.
	local files="$(ls -Q "$address")"
	eval file_arr="($files)"

	# lokalna iteracijska spremenljivka za iteriranje po datotekah v podanem direktoriju
	local f
	for f in "${file_arr[@]}" ; do
		# Shrani tip trenutne datoteke.
		...
		if [[ "$file_type" == "directory" ]] ; then
			...
			# Izracunaj pot direktorija.
			local new_address="$address""/$f"
			# Ce se nisi dosegel maksimalne globine...
...
# izpis drevesne strukture podanega direktorija
	...
	max_depth=3
	# Shrani ciljni naslov in maksimalno globino iskanja.
	# Ce ni podan ne naslov ne maksimalna globina... (privzete vrednosti)
	if (( $# == 1 )); then
		target_address="$PWD"
	# Ce je podana samo maksimalna globina iskanja... (privzeta vrednost za pot)
	elif [[ $# -eq 2 && $2 =~ ^-?[0-9]+$ ]] ; then
		max_depth=$2
		target_address="$PWD"
	# Sicer preberi naslov (in maksimalno globino iskanja, ce je podana).
	else
		target_address=""
		for (( i = 2; i <= $#; i++ )); do
			if [[ ${!i} =~ ^-?[0-9]+$ ]] ; then
	...
	# Izpisi direktorij, katerega beres.
	printf "DIR   %s\n" "$target_address"
...
# izpis prostorskih statistik podanega direktorija
	...
	max_depth=3
	# glej drevo (enako procesiranje argumentov)
	if (( $# == 1 )); then
