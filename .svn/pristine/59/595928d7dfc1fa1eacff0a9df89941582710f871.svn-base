...
function drevo()
{
	...
	# printf "max_depth == %s current_depth == %s\n" "$max_depth" "$current_depth"

	local files=$(ls $address)
	local f
	for f in $files ; do
		local file_type_raw=$(file $address"/"$f)

		local temp_arr=($file_type_raw)
		local file_type=${temp_arr[1]}
...
# izpis drevesne strukture podanega direktorija
	...
	target_address=${2:-$PWD}
	printf "DIR %s\n" "$target_address"
	drevo $target_address 4 1
# izpis prostorskih statistik podanega direktorija
	...
	res=$(velikost $target_address 4 0 0)
	...
	# izracunaj rezultat za podani direktorij
	res=$(bloki $target_address 4 0 0)
	# pristej se statistiko podanega direktorija samega
	...
	# izracunaj rezultat za podani direktorij
	res=$(bloki_velikost $target_address 4 0 0)
	# pristej se statistiko podanega direktorija samega
	blocks_folder=$(stat --printf="%b" $target_address)
